...
class DnDataset(Dataset):
    def __init__(self, ifn, params):
        self.psz = params['train']['psz']
        ds_cfg = params['dataset']

        N = h5py.File(ifn, 'r')[ds_cfg['xkey']].shape[0]
        ...
        if ds_cfg['norm']:
            if ds_cfg['mean4norm'] is None:
                ...
                logging.info(f'features from {ifn} are normalized with computed mean: {_avg}, std: {_std}')
            else:
                self.features = ((self.features - ds_cfg['mean4norm']) / ds_cfg['std4norm']).astype(np.float32)
                logging.info(f"features from {ifn} are normalized with provided mean: {ds_cfg['mean4norm']}, std: {ds_cfg['std4norm']}")
